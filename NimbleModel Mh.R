NimModel <- nimbleCode({
  ...
  beta0.p.y ~ dlogis(0,1) #detection probability intercept
  sd.p.y ~ dunif(0,3) #detection probability sd across individuals on logit scale
  ...
  for(i in 1:M){
    ...
    # z.y[i] ~ dnorm(0,sd=sd.p.y)
    # logit(p.y[i]) <- beta0.p.y + z.y[i]
    ...
    y.true[i,1:K] ~ dHurdleVector(p.y=p.y[i],lambda.y=lambda.y,K=K,z=z[i])
  }
