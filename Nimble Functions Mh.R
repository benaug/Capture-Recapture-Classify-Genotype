...
IDSampler <- nimbleFunction(
  ...
  run = function() {
      ...
      if(z[i]==1){
        for(k in 1:K){
          if(y.true[i,k]==0){ #if not captured
            lp.y[i,k] <- dbinom(0,size=1,prob=p.y[i],log=TRUE)
          }else{ #if captured
            lp.y[i,k] <- dbinom(1,size=1,prob=p.y[i],log=TRUE) + log(dpois(y.true[i,k],lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
          }
        }
      }
    }
    ...
    for(l in 1:n.samples){
      ...
      #get g probs first
      lp.G.prop <- rep(0,M) #considers how well sample l matches all G.true in population. Can compute this once
      for(i in 1:M){
        if(z[i]==1){
          for(m in 1:n.loci){
            for(rep in 1:n.rep){
      ...
      for(i in 1:M){
        if(z[i]==1&(!(i==ID[l]))){
          ...
          if(y.tmp1==0){ #if not captured
            lp.y.prop[i] <- dbinom(0,size=1,prob=p.y[i],log=TRUE)
          }else{ #if captured
            lp.y.prop[i] <- dbinom(1,size=1,prob=p.y[i],log=TRUE) + 
              log(dpois(y.tmp1,lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
          }
          if(y.tmp2==0){ #if not captured
            lp.y.prop[i] <- lp.y.prop[i] + dbinom(0,size=1,prob=p.y[ID[l]],log=TRUE)
          }else{ #if captured
            lp.y.prop[i] <- lp.y.prop[i] + dbinom(1,size=1,prob=p.y[ID[l]],log=TRUE) + 
      ...
      lp.G.prop.for <- lp.G.prop
      lp.G.prop.for[ID[l]] <- -Inf #disallow self proposal
      prop.probs <- exp(lp.G.prop.for + lp.y.prop)
      prop.probs <- prop.probs/sum(prop.probs)
      prop.i <- rcat(1,prob=prop.probs)
      focal.i <- ID[l]
      
      if(ID[l]!=prop.i){ #skip if propose same ID. Actually, this is disallowed now
        #update this ID
        ID.cand[l] <- prop.i
        ...
        for(i in 1:2){
          if(y.cand[swapped[i],this.k[l]]==0){ #if not captured
            lp.y.cand[swapped[i],this.k[l]] <- dbinom(0,size=1,prob=p.y[swapped[i]],log=TRUE)
          }else{ #if captured
            lp.y.cand[swapped[i],this.k[l]] <- dbinom(1,size=1,prob=p.y[swapped[i]],log=TRUE) + log(dpois(y.cand[swapped[i],this.k[l]],lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
          }
        }
        #update lp.G
        for(m in 1:n.loci){
        ...
        for(i in 1:M){
          if(z[i]==1&(!(i==ID.cand[l]))){
            y.tmp1 <- y.cand[i,this.k[l]] + 1 #if we add sample here
            y.tmp2 <- y.cand[ID.cand[l],this.k[l]] - 1 #if we add sample here
            if(y.tmp1==0){ #if not captured
              lp.y.prop.back[i] <- dbinom(0,size=1,prob=p.y[i],log=TRUE)
            }else{ #if captured
              lp.y.prop.back[i] <- dbinom(1,size=1,prob=p.y[i],log=TRUE) + 
                log(dpois(y.tmp1,lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
            }
            if(y.tmp2==0){ #if not captured
              lp.y.prop.back[i] <- lp.y.prop.back[i] + dbinom(0,size=1,prob=p.y[ID.cand[l]],log=TRUE)
            }else{ #if captured
              lp.y.prop.back[i] <- lp.y.prop.back[i] + dbinom(1,size=1,prob=p.y[ID.cand[l]],log=TRUE) + 
                log(dpois(y.tmp2,lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
            }
          }else{ #can't propose this guy if z==0
        ...
        lp.G.prop.back <- lp.G.prop
        lp.G.prop.back[ID.cand[l]] <- -Inf #disallow self proposal
        prop.probs.back <- exp(lp.G.prop.back + lp.y.prop.back)
