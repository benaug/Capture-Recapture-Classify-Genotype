        ...
        if(z[i]==1){
          ...
          if(i!=ID[l]){#otherwise lp.y.total is already correct (current total)
            ...
            if(y.tmp1==0){ #if not captured
              lp.tmp1 <- dbinom(0,size=1,prob=p.y[i],log=TRUE)
            }else{ #if captured
              lp.tmp1 <- dbinom(1,size=1,prob=p.y[i],log=TRUE) + log(dpois(y.tmp1,lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
            }
            #new guy
            y.tmp2 <- y.true[i,this.k[l]] + 1
            lp.tmp2 <- 0
            if(y.tmp2==0){ #if not captured, will never happen in giving a guy this sample
              lp.tmp2 <- dbinom(0,size=1,prob=p.y[i],log=TRUE)
            }else{ #if captured
              lp.tmp2 <- dbinom(1,size=1,prob=p.y[i],log=TRUE) + log(dpois(y.tmp2,lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
            }
      ...
      if(ID[l]!=prop.i){ #skip if propose same ID
        ...
        for(i in 1:2){
          if(y.true[swapped[i],this.k[l]]==0){ #if not captured
            lp.y[swapped[i],this.k[l]] <- dbinom(0,size=1,prob=p.y[i],log=TRUE)
          }else{ #if captured
            lp.y[swapped[i],this.k[l]] <- dbinom(1,size=1,prob=p.y[i],log=TRUE) + log(dpois(y.true[swapped[i],this.k[l]],lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
          }
        }
      }
    }
