#Mb can take longer to converge, N estimate in Mb
#only informed by first capture events

#You could add individual heterogeneity and time effects to this one
#without modifying custom ID update. Need to modify model file and
#dHurdleVectorMb custom distribution as long as you keep it as a vector node.
#unvectorizing would require change to custom ID update

library(coda)
library(nimble)
source("sim.data.Mb.R")
source("init.data.R")
source("build.genos.R")
source("map.genos.R")
source("NimbleModel Mb.R")
source("Nimble Functions Mb.R")
...
p.y <- c(0.1,0.4) #capture probability, 1st and subsequent capture
#expected number of samples given capture (ZT Poisson), 1st and subsequent capture
#model file is set up to not vary lambda.y, but can be easily modified to allow it
#probably hard to support 2 parameters on both p.y and lambda.y without a lot of data
lambda.y <- c(1,1)
...
data <- sim.data.Mb(N=N,p.y=p.y,lambda.y=lambda.y,K=K,#cap-recap parameters/constants
...
Niminits <- list(z=nimbuild$z,N=nimbuild$N, #must initialize N to be sum(z) for this data augmentation approach
                 log_lambda.N=log(nimbuild$N), #don't have to provide this init, but this speeds convergence a bit
...
#If you set the model file up for a behavioral response for lambda.y, change 'lambda.y[1]' to 'lambda.y' on next line
config.nodes <- c('log_lambda.N','logit_p.y','lambda.y[1]','p.geno.het','p.geno.hom','gammaMat')
# config.nodes <- c()
...
#In Mb, only first cap prob is correlated with lambda.N, so not including recapture probability
# conf$removeSampler(c("log_lambda.N","logit_p.y[1]"))
conf$addSampler(target = c("log_lambda.N","logit_p.y[1]"),
