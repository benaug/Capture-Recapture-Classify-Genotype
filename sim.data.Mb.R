sim.data.Mb <-
  function(N=NA,p.y=NA,lambda.y=NA,K=NA,p.geno.het=NA,
    ...
    if(length(p.y)!=2)stop("Must supply both 1st and subsequent capture probability in p.y for Mb")
    if(length(lambda.y)!=2)stop("Must supply both 1st and subsequent capture probability in lambda.y for Mb")
    ...
    for(i in 1:N){
      cap.type <- 1
      for(k in 1:K){
        y.det[i,k] <- rbinom(1,1,p.y[cap.type])
        if(y.det[i,k]==1){
          y.count[i,k] <- rzapois(1,lambda.y[cap.type],pobs0=0)
          cap.type <- 2
        }
      }
    }
    ...
    out <- list(y=y,this.k=this.k,G.true=G.true,G.obs=G.error,n.loci=n.loci,n.levels=n.levels,
             ...
             ID=ID,K=K,n=nrow(y),G.obstype=G.obstype,seed=seed)
