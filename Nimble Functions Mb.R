...
dHurdleVectorMb <- nimbleFunction(
  run = function(x = double(1),p.y=double(1),lambda.y=double(1),z=double(0),K=double(0), log = integer(0)) {
    returnType(double(0))
    logProb <- 0
    if(z==1){
      cap.type <- 1
      for(k in 1:K){
        if(x[k]==0){ #if not captured, Bernoulli
          logProb <- logProb + dbinom(0,size=1,prob=p.y[cap.type],log=TRUE)
        }else{ #if captured, Bernoulli and ZT Poisson
          logProb <- logProb + dbinom(1,size=1,prob=p.y[cap.type],log=TRUE) + 
            log(dpois(x[k],lambda=lambda.y[cap.type])/(1-dpois(0,lambda=lambda.y[cap.type])))
          cap.type <- 2 #set cap.type=2 after first capture
        }
      }
    }
    if(log){
...
rHurdleVectorMb <- nimbleFunction(
  run = function(n = integer(0),p.y=double(1),lambda.y=double(1),z=double(0),K=double(0)) {
    returnType(double(1))
    return(rep(0,K))
  }
)
...
IDSampler <- nimbleFunction(
  ...
  run = function() {
    ...
    theta <- model$theta
    p.y <- model$p.y[1:2]
    lambda.y <- model$lambda.y[1:2]
    
    #For Mb, I'm going to use the likelihood for all occasions, not just this.k[l]
    #because updating a sample at one k can change the likelihood of all captures after it, too
    lp.y <- model$logProb_y.true[,1]
    ...
    for(l in 1:n.samples){
      ...
      for(i in 1:M){
        if(z[i]==1){
          ...
          if(i!=ID[l]){#otherwise lp.y.total is already correct (current total)
            ...
            #old guy
            y.tmp1 <- y.true[ID[l],]
            y.tmp1[this.k[l]] <- y.tmp1[this.k[l]] - 1
            #several occasions likelihood may change with updating 1 sample in Mb
            lp.tmp1 <-  dHurdleVectorMb(x=y.tmp1,p.y=p.y[1:2],lambda.y=lambda.y[1:2],K=K,z=1,log=TRUE)
            #new guy
            y.tmp2 <- y.true[i,]
            y.tmp2[this.k[l]] <- y.tmp2[this.k[l]] + 1
            lp.tmp2 <-  dHurdleVectorMb(x=y.tmp2,p.y=p.y[1:2],lambda.y=lambda.y[1:2],K=K,z=1,log=TRUE)
            #add likelihood difference for each guy from current.
            lp.y.total[i] <- lp.y.total[i] + (lp.y[ID[l]] - lp.tmp1) + (lp.y[i] - lp.tmp2)
          }
      ...
      if(ID[l]!=prop.i){ #skip if propose same ID
        ...
        ##update lp.y
        for(i in 1:2){
          lp.y[swapped[i]] <-  dHurdleVectorMb(x=y.true[swapped[i],],p.y=p.y[1:2],lambda.y=lambda.y[1:2],K=K,z=1,log=TRUE)
        }
      }
    }
