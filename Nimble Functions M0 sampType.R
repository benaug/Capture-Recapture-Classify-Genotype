      ...
      if(z[i]==1){
        for(k in 1:K){
          if(y.true[i,k]==0){ #if not captured
            ...
            lp.y[i,k] <- dbinom(1,size=1,prob=p.y,log=TRUE) + 
              log(dpois(y.true[i,k],lambda=lambda.y)/(1-dpois(0,lambda=lambda.y)))
          }
        }
      }
    }
      ...
      for(m in 1:n.loci){
        for(rep in 1:n.rep){
          if(na.ind[l,m,rep]==FALSE){
            lp.G[l,m,rep] <- dcat(G.obs[l,m,rep],theta[m,samp.type[l],G.true[ID[l],m],1:n.levels[m]],log=TRUE)
          }
        }
      }
    }
    ...
    for(l in 1:n.samples){
          ...
          for(m in 1:n.loci){
            for(rep in 1:n.rep){
              if(na.ind[l,m,rep]==FALSE){ #if observed
                lp.G.prop[i] <- lp.G.prop[i] + log(theta[m,samp.type[l],G.true[i,m],G.obs[l,m,rep]])
              }
            }
          }
        }
      ...
      if(ID[l]!=prop.i){ #skip if propose same ID. Actually, this is disallowed now
        ...
        for(m in 1:n.loci){
          for(rep in 1:n.rep){
            if(na.ind[l,m,rep]==FALSE){
              lp.G.cand[l,m,rep] <- dcat(G.obs[l,m,rep],theta[m,samp.type[l],G.true[ID.cand[l],m],1:n.levels[m]],log=TRUE)
            }else{
              lp.G.cand[l,m,rep] <- 0
            }
          }
        ...
        if(runif(1)<exp((sum(lp.y.cand[swapped,this.k[l]]) + sum(lp.G.cand[l,,]))-
          ...
          lp.G.cand[l,,] <- lp.G[l,,]
          ID.cand[l] <- ID[l] #need this for select probs
        }
      }
    }
