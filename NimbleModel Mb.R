NimModel <- nimbleCode({
  ...
  for(i in 1:2){
    logit(p.y[i]) ~ dlogis(0,1) #first detection probability
    # lambda.y[i] ~ dunif(0,5) #behavioral response might increase number of samples|detection
  }
  #assuming lambda.y unchanged by first detection here
  lambda.y[1] ~ dunif(0,5) #parameter for number of samples|detection
  lambda.y[2] <- lambda.y[1]
  ...
  for(i in 1:M){
    ...
    y.true[i,1:K] ~ dHurdleVectorMb(p.y=p.y[1:2],lambda.y=lambda.y[1:2],K=K,z=z[i])
  }
