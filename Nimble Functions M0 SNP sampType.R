      ...
      for(i2 in 1:length(these.samps)){
        for(obs in 1:n.rep){
          if(na.ind[these.samps[i2],obs]==FALSE){ #if observed
            error.probs <- error.probs*model$theta[samp.type[these.samps[i2]],1:3,G.obs[these.samps[i2],m,obs]]
          }
        }
      }
      ...
      for(m in 1:n.loci){
        for(rep in 1:n.rep){
          if(na.ind[l,m,rep]==FALSE){
            lp.G[l,m,rep] <- dcat(G.obs[l,m,rep],theta[samp.type[l],G.true[ID[l],m],1:3],log=TRUE)
          }
        }
      }
    }
    ...
    for(l in 1:n.samples){
          ...
          for(m in 1:n.loci){
            for(rep in 1:n.rep){
              if(na.ind[l,m,rep]==FALSE){ #if observed
                lp.G.prop[i] <- lp.G.prop[i] + log(theta[samp.type[l],G.true[i,m],G.obs[l,m,rep]])
              }
            }
          }
        }
      ...
      if(ID[l]!=prop.i){ #skip if propose same ID. Actually, this is disallowed now
        ...
        for(m in 1:n.loci){
          for(rep in 1:n.rep){
            if(na.ind[l,m,rep]==FALSE){
              lp.G.cand[l,m,rep] <- dcat(G.obs[l,m,rep],theta[samp.type[l],G.true[ID.cand[l],m],1:3],log=TRUE)
            }else{
              lp.G.cand[l,m,rep] <- 0
            }
          }
